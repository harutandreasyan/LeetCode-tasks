/*
    Given the root of a binary tree, return the leftmost value in the last row of the tree.

    Example 1:
    Input: root = [2,1,3]
    Output: 1

    Example 2:
    Input: root = [1,2,3,4,null,5,6,null,null,7]
    Output: 7

    Constraints:
        The number of nodes in the tree is in the range [1, 10^4].
        -2^31 <= Node.val <= 2^31 - 1
*/

import { buildTreeFromArray } from '../Binary-Tree/helpers.js'

/* Time - O(n), Space - O(n) */
// BFS

var findBottomLeftValue = function (root) {
    if (!root) return null
    const queue = [root]
    let leftMost = root.val

    while (queue.length) {
        const size = queue.length
        leftMost = queue[0].val 

        for (let i = 0; i < size; i++) {
            const node = queue.shift()
            if (node.left) queue.push(node.left)
            if (node.right) queue.push(node.right)
        }
    }

    return leftMost
}

let root1 = buildTreeFromArray([2, 1, 3])
console.log(findBottomLeftValue(root1)) // 1

let root2 = buildTreeFromArray([1, 2, 3, 4, null, 5, 6, null, null, 7])
console.log(findBottomLeftValue(root2)) // 7

let root3 = buildTreeFromArray([9, 10, 1, 3, 5, null, null, null, null, 14, 35])
console.log(findBottomLeftValue(root3)) // 14
